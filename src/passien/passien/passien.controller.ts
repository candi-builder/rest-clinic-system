...
export class PassienController {
	constructor(
		private passienService: PassienService,
	...
	async getListPassien(
		...
		const passien = await this.passienService.getListPassienByPoliId(
			page,
			size,
			poliId,
		);
	...
	@Delete(":passienId")
	@HttpCode(200)
	async deletePassienById(
		@Param("passienId") passienId: number,
