...
export class DoctorService {
	...
	async updatePasienStatus(
		passienId: number,
	): Promise<BaseResponse<DoctorResponse>> {
	...
	async createDiagnosa(
		pasienId: number,
		request: DiagnosaRequest,
	): Promise<BaseResponse<DiagnosaRequest>> {
		try {
			const validateRequest = this.validationService.validate(
				DoctorValidation.DIAGNOSA,
				request,
			);
			...
			if (existingDiagnosa) {
				throw new HttpException(
					"Diagnosa untuk pasien ini sudah ada",
					HttpStatus.BAD_REQUEST,
				);
			}
